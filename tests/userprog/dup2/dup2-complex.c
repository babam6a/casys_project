...
main (int argc UNUSED, char *argv[] UNUSED) {
  ...
  //check file is closed if dup2 occured
  ...
  close (fd1);
  close (fd2);
  //after close, file in fd1 closed, file in fd2 is not closed(alive in fd3);
  ...
  if (!(pid = fork ("child"))){ // child
    // at this time, (fd1, fd2 is for file in fd2, fd4 = cheer, fd6 = up, fd3 = stdout)
    msg ("child begin");
    close (fd1);
    close (fd2); //file in fd2 closed
    dup2 (fd4, fd2);
    dup2 (fd3, fd1);
    ...
    close (fd1);
    close (fd2);
    close (fd3); // close stdout
    close (fd4);
    close (fd5); // close NULL
    close (fd6);
    seek (fd4, 0); // seek NULL
