...
void *load_segment (FILE *file, uint64_t section_ofs, uint64_t code_size) {
	...
	/* mmap page */
	void *upage = (void *)phdrs[0].p_vaddr; // it must be aligned :)
	void *kpage = (void *)mmap((void *)upage, PGSIZE, protect_flag, map_flag, -1, 0);
	if (kpage == MAP_FAILED) {
		printf("mmap failed!\n");
		return false;
	}

	print_mapping_info((uint64_t)upage, 0, PGSIZE);

    uint64_t load_addr = phdrs[0].p_vaddr;
    uint32_t size = phdrs[0].p_filesz;
    uint32_t ofs = phdrs[0].p_offset;
    ...
    int flags = set_flags(phdrs[0].p_flags);

    if (mprotect((void *)load_addr, PGSIZE, flags) == -1) {
        printf("mprotect error\n");
        goto error;
    }

    /* for static file, we should allocate some spaces */
    Elf64_Phdr target_seg = phdrs[load_phdr_num - 1];
    uint64_t segment_start = target_seg.p_vaddr;
    uint64_t base_addr = target_seg.p_vaddr & (~PGMASK);
    uint64_t segment_size = 0x3000 - (segment_start - base_addr);
    uint64_t segment_ofs = target_seg.p_offset;

    void *segment_base = (void *)mmap((void *)base_addr, 0x3000, protect_flag, map_flag, -1, 0);
	if (segment_base == MAP_FAILED) {
		printf("mmap failed!\n");
		goto error;
	}

    print_mapping_info((uint64_t)segment_base, 0, 0x3000);

    fseek(file, segment_ofs, SEEK_SET);

    if (fread ((void *)segment_start, 1, segment_size, file) != segment_size) {
        printf("memcpy error\n");
        goto segment_error;
    }

    flags = set_flags(phdrs[0].p_flags);

    if (mprotect(segment_base, 0x3000, flags) == -1) {
        printf("mprotect error\n");
        goto segment_error;
    }
...
void page_fault(int sig, siginfo_t *info, void *context) {
    ...
    /* find segment related to segfault addr */
    for (int i = 0; i < load_phdr_num; i++) {
        uint64_t base = phdrs[i].p_vaddr;
        uint64_t file_size = phdrs[i].p_filesz;
        uint64_t mem_size = phdrs[i].p_memsz;

        if ((base <= segfault_addr) && (segfault_addr <= base + file_size)) {
            memcpy(&target_segment, &phdrs[i], sizeof(Elf64_Phdr));
            found = true;
            break;
        }
        /* if it is bss section */
        else if ((file_size != mem_size) && (base + file_size < segfault_addr)) {
            memcpy(&target_segment, &phdrs[i], sizeof(Elf64_Phdr));
            found = true;
            bss = true;
            break;
    ...
    if (bss) {
        ...
        uint64_t segment_start = target_segment.p_vaddr;
        uint64_t segment_end = segment_start + target_segment.p_filesz;
        uint64_t segfault_start = segfault_addr & (~PGMASK);
        uint64_t segfault_end = segfault_start + PGSIZE;
        uint64_t size = PGSIZE;
        uint64_t read_start = segfault_start;

        if (segfault_start < segment_start) {
            size -= (segment_start - segfault_start);
            read_start = segment_start;
        }
        if (segment_end < segfault_end) {
            size -= (segfault_end - segment_end);
        }

        /* mmap page */
        void *segfault_mmap = (void *)mmap((void *)segfault_start, PGSIZE, protect_flag, map_flag, -1, 0);
        if (segfault_mmap == MAP_FAILED) {
            write(STDERR_FILENO, "mmap in bss error\n", 19);
            goto error;
        }
        ...
        if (read(fd, (void *)read_start, size) != size) {
            write(STDOUT_FILENO, "write in bss error\n", 20);
            goto after_open_error;
        }
